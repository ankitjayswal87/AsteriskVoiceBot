		...
		while True:
			...
			if(event_type=='StasisStart'):
				...
				if channel_name=='PJSIP':
					print("IncomingChannelID:"+channelid)
					# creating external channel
					...
					# creating mixing type bridge
					bridge_id = ari.create_bridge("test_bridge","mixing")
					print("BridgeID:"+bridge_id)

					# creating snoop channel
					snoopid = uuid.uuid4()
					snoop_channel_id = ari.create_snoop_channel(channelid,snoopid,"both","both",cfg.APP)
					print("SnoopID:"+snoop_channel_id)

					# setting a key 'port:15605' and its values
					...
					# adding snoop channel and external channel into bridge
					ari.add_channel_in_bridge(bridge_id,snoop_channel_id)
					ari.add_channel_in_bridge(bridge_id,external_media_channelid)

					# continue in dialplan with incoming SIP channnel
